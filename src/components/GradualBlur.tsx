...
const useResponsiveDimension = <K extends keyof GradualBlurProps>(
  ...
  const [val, setVal] = useState<GradualBlurProps[K] | undefined>(config[key as K] as GradualBlurProps[K] | undefined);
  useEffect(() => {
    if (!responsive) return;
    const calc = () => {
      const w = window.innerWidth;
      let v = config[key as K] as GradualBlurProps[K] | undefined;
      const cap = (s: string) => s.charAt(0).toUpperCase() + s.slice(1);
      const k = cap(key as string);
      const cfg = config as Record<string, unknown>;
      if (w <= 480 && cfg['mobile' + k] != null) v = cfg['mobile' + k] as GradualBlurProps[K];
      else if (w <= 768 && cfg['tablet' + k] != null) v = cfg['tablet' + k] as GradualBlurProps[K];
      else if (w <= 1024 && cfg['desktop' + k] != null) v = cfg['desktop' + k] as GradualBlurProps[K];
      setVal(v);
    };
  ...
  return responsive ? val : (config[key as K] as GradualBlurProps[K] | undefined);
};
...
type GradualBlurMemoComponent = React.MemoExoticComponent<
  React.FC<PropsWithChildren<GradualBlurProps>>
> & {
